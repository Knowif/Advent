...
namespace Advent
{
	partial class GameMap
	{
		static void BuildCampus()
		{
			...
			AObject Pond = new AObject(
				"池塘", new[] { "水池", "水", "鱼" },
				desc:	"池塘在体育馆东门外延伸成L形。略微浑浊的水面被风吹成网状褶皱，天空的颜色在其中破碎。几只模糊的鱼在水中移动。");
			...
			SetupArea(ref HiSq, "寝室楼前的小广场",
				...
				usable:		new[] { Building6, Building8, Pond, Flower },
				notClear:	new[] { Building1, Building7 },
				godir:		new Dictionary<Direction, Area>
							{ { Direction.E, A7t8 },
							  { Direction.W, EGym },
							  { Direction.N, FrHiSq },
			...
			SetupArea(ref BkHiSq, "台阶",
				...
				notClear:	new[] { Building6, Building8, Building10, Pond },
				godir:		new Dictionary<Direction, Area>
							{ { Direction.N, HiSq } });
			...
			SetupArea(ref LoSq, "教学楼间的小广场",
				...
				usable:		new[] { Building1, Building2, Building3, doorLoSq_1, doorLoSq_2, doorLoSq_3 },
			...
			SetupArea(ref EGym, "体育馆边",
				desc:		"你站在体育馆边上。小广场在你东侧，西边则是体育馆[6#]的侧门，门前两边各有一个长方形池塘。你也可以往西北绕过侧门来到正门前的平地上，或是向南绕过花坛边缘，那边远处是一排寝室楼。",
				usable:		new[] { Building6, Pond, doorEGym },
			...
			SetupArea(ref BkGym, "体育馆后的空地",
				...
				usable:		new[] { Building6, Flower, Building11, Pond, doorBkGym },
			...
			SetupArea(ref SE6Brg, "木板桥上",
				...
				usable:		new[] { Building6, Flower, Pond, Building11 },
